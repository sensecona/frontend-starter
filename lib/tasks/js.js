    ...
    run: function(taskParams) {
      ...
      if (!injector.isCanceled) {
        if (isWebpack) {
          ...
          if (taskParams.isApp) {
            ...
            var vendorGlob = comp.getGlob('vendor');
            ...
            stream = gulp.src(stream ? stream : mainBowerFiles(injector.taskConfig.mainBowerFiles), { base: dirs.src.main })
              ...
              .pipe(vendorGlob.length === 0 ? through.obj() : gulp.src(comp.getGlob('vendor'), { base: dirs.src.main }));
          }
        }
      ...
      if (!isWebpack) {
        if (taskParams.isApp && injector.taskConfig.concatVendorApp) {
          ...
          if (!injector.isCanceled) {
            stream = stream.pipe(gulp.src(vendorDir + filenameVendor, { passthrough: true, allowEmpty: true}));
